...
os.environ['TORCH_SCATTER_DETERMINISTIC'] = '1'
...
# orginal version of STACAME with GAN loss and auxiliary model
def train_STACAME_GAN(adata_species_dict,
    ...
    species_ids = sorted(adata_species_dict.keys())
    ...
    for sp in species_ids:
        species_add_dict[sp] = k_add
        k_add += adata_species_dict[sp].n_obs

    adata_first = adata_species_dict[species_ids[0]]
    edge_ndarray = np.array([adata_first.uns['edgeList'][0], adata_first.uns['edgeList'][1]])
    for sp in species_ids[1:]:
        adata = adata_species_dict[sp]
        edge_arr_temp = np.array([adata.uns['edgeList'][0], adata.uns['edgeList'][1]]) + species_add_dict[sp]
    ...
    # Concatenate latent embeddings X (z) from all species
    X = z_dict[species_ids[0]].cpu().detach().numpy()
    for sp in species_ids[1:]:
        X = np.concatenate((X, z_dict[sp].cpu().detach().numpy()), axis=0)

    z = torch.FloatTensor(X)

    # Concatenate original features merge_X across species
    if 'highly_variable' in adata_first.var.columns:
        merge_X = adata_first[:, adata_first.uns['highly_variable']].X.todense()
    else:
        merge_X = adata_first.X.todense()
    for sp in species_ids[1:]:
        adata = adata_species_dict[sp]
        if 'highly_variable' in adata.var.columns:
    ...
    sc.pp.scale(adata_X)
    sc.tl.pca(adata_X, n_comps=concate_pca_dim, random_state=random_seed)
    ...
    auxiliary_D_Z = STACAME.MultiClassDiscriminator(hidden_dims[1], len(species_ids)).to(device)
    auxiliary_optimizer_D = torch.optim.Adam(list(auxiliary_D_Z.parameters()), lr=0.001, weight_decay=0.001, foreach=False)
    auxiliary_D_Z.train()

    D_Z = STACAME.MultiClassDiscriminator(hidden_dims[1], len(species_ids)).to(device)
    optimizer_D = torch.optim.Adam(list(D_Z.parameters()), lr=0.001, weight_decay=0.001, foreach=False)
    D_Z.train()

    species_list = []
    for sp in species_ids:
        species_list.extend([sp] * adata_species_dict[sp].n_obs)
    ...
    for epoch in tqdm(range(0, n_epochs_species)): 
        k_add = 0
        for sp in species_ids:
            species_add_dict[sp] = int(k_add)
            adata_species_dict[sp].obsm['STAGATE'] = z[
                k_add:int(k_add + adata_species_dict[sp].n_obs), :].cpu().detach().numpy()
            z_dict[sp] = adata_species_dict[sp].obsm['STAGATE']
            k_add = int(k_add + adata_species_dict[sp].n_obs)
        ...
        mmd_loss_sum = 0
        for sp in species_ids:
            k_add = species_add_dict[sp]
            remain_list = sorted(list(
                set(list(range(z.shape[0]))) - set(range(k_add, int(k_add + adata_species_dict[sp].n_obs)))))
            epoch_rng = random_module.Random(random_seed + epoch)
            ind_1 = epoch_rng.sample(list(range(k_add, int(k_add + adata_species_dict[sp].n_obs))), mmd_batch_size)
            ind_2 = epoch_rng.sample(remain_list, mmd_batch_size)
        ...
        if epoch % 100 == 0:
            ...
            for batch_pair in mnn_dict.keys():
                ...
                for anchor in mnn_dict[batch_pair].keys():
                    ...
                    positive_list.append(positive_spot)
                    section_size = len(cellname_by_batch_dict[batchname_list[anchor]])
                    rng = np.random.RandomState(random_seed + epoch)
                    negative_list.append(
                        cellname_by_batch_dict[batchname_list[anchor]][rng.randint(section_size)])
        ...
        for sp in species_ids:
            k_add = species_add_dict[sp]
            adata_species_dict[sp].obsm[key_added] = z[
                k_add:int(k_add + adata_species_dict[sp].n_obs), :].cpu().detach().numpy()
...
# Subgraph version of train_STACAME_GAN
...
# # Subgraph version of train_STACAME
...
## Subgraph version of train_STACAME_GAN without auxiliary model
